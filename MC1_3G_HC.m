...
for sth = 1:num_simulations
    ...
    X = inputMCXG3_HC((sth-1)*N*T+1:sth*N*T, :);
    Y = inputMCYG3_HC((sth-1)*N*T+1:sth*N*T);
...
fileID1 = fopen('assignment_G3_HC.txt', 'w');
fileID2 = fopen('theta_G3_HC.txt', 'w');

for sth = 1:500
    fprintf(fileID1, '%d ', opt_group_assign_all(sth, :));
    fprintf(fileID1, '\n');
    fprintf(fileID2, '%.4f ', opt_thetas_assign_all(sth, g, :));
    fprintf(fileID2, '\n');
end
