...
load('inputMCXG10_HC.txt');
load('inputMCYG10_HC.txt');
load('BigG_perm_G10_HC');
...
for sim = 1:repNum
    X = inputMCXG10_HC((sim-1)*N*T+1:sim*N*T, :); % Obtain the data for X for all countries and periods in the current simulation
    Y = inputMCYG10_HC((sim-1)*N*T+1:sim*N*T); % Obtain the data for Y for all countries and periods in the current simulation
...
true_thetas = [
    0.3982; 0.042; 0.3982; 0.042; 0.3475; 0.044; 0.334; 0.0515; 0.2815;0.0666;0.2779;0.0665;0.346;0.0657;0.3684;0.0626;0.3852;0.0623;0.3997;0.0637];
